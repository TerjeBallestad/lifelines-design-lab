...
export function buildDocHtml(docId, labContent) {
  ...
  const runsHtml = renderRuns(runs);
...
export function buildStationeryHtml(kind) {
  ...
  return pageShell({ kind, styleCss: template.styleCss, bodyHtml });
}
...
async function main(argv) {
  ...
  if (!docId) throw new Error('Usage: render-doc.mjs <docId> [outDir] | --stationery KIND [outDir]');

  const { buildTinyOlsenArtifacts } = await import(
    '../../scripts/blueprint/generate-tiny-olsen-case.mjs'
  );
